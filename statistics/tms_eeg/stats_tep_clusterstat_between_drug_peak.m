%##### COMPARE CHANGES IN TEPS BETWEEN DRUG CONDITIONS AT CERTAIN PEAKS #####

% This script compares changes in  TEPs (post-pre) between drug conditions
% using cluster-based permutation statistics as implemented in FieldTrip. 
% Data are compared across narrow time windows around main peaks.
% Comparisons are run separately for each site (see comparisons of interest
% below - comment out the site not interested in).
% Inputs are EEG data in grand average structures generated by FieldTrip.
% Note that the script create_grandAverage_diff.mat needs to be run before
% running this script.
...
comparisons = {{'C1','pfc'},{'C2','pfc'}}; %PFC comparisons
% comparisons = {{'C1','ppc'},{'C2','ppc'}}; %PPC comparisons
...
site = comparisons{1,1}{2};

TOI = [];
for latx = 1:length(latencies.(site))
    if latencies.(site)(latx)<100
        N=5;
    else
        N=15;
    end
    TOI{latx,1}(1,1) = (latencies.(site)(latx)-N)./1000;
    TOI{latx,1}(1,2) = (latencies.(site)(latx)+N)./1000;
end
...
load([pathIn,'grandAverage_',dataAv,'.mat']);

% Generate required input matrices
if length(comparisons{1}) == 3
    for a = 1:size(comparisons,1) % loop over number of comparisons
        if isstruct(TOI)
          for b = 1:size(peakNames,1) % loop over TOIs
                saveNameTemp{a,b} = [comparisons{a,1}{1,1},'_',comparisons{a,1}{1,2},'_',comparisons{a,1}{1,3},'_VS_',comparisons{a,2}{1,1},'_',comparisons{a,2}{1,2},'_',comparisons{a,2}{1,3},'_TOI_',peakNames{b},'_',dataAv];
                condition1Temp{a,b} = comparisons{a,1};
                condition2Temp{a,b} = comparisons{a,2};
                toiRangeTemp{a,b} = TOI.(comparisons{a,2}){b};
            end
        else
            for b = 1:size(TOI,1) % loop over TOIs
                saveNameTemp{a,b} = [comparisons{a,1}{1,1},'_',comparisons{a,1}{1,2},'_',comparisons{a,1}{1,3},'_VS_',comparisons{a,2}{1,1},'_',comparisons{a,2}{1,2},'_',comparisons{a,2}{1,3},'_TOI_',num2str(TOI{b,1}(1,1)),'_',num2str(TOI{b,1}(1,2)),'_',dataAv];
                condition1Temp{a,b} = comparisons{a,1};
                condition2Temp{a,b} = comparisons{a,2};
...
elseif length(comparisons{1}) == 2
    for a = 1:size(comparisons,1) % loop over number of comparisons
        if isstruct(TOI)
            for b = 1:size(peakNames,1) % loop over TOIs
                saveNameTemp{a,b} = [comparisons{a,1}{1,1},'_',comparisons{a,1}{1,2},'_VS_',comparisons{a,2}{1,1},'_',comparisons{a,2}{1,2},'_TOI_',peakNames{b},'_',dataAv];
                condition1Temp{a,b} = comparisons{a,1};
                condition2Temp{a,b} = comparisons{a,2};
                toiRangeTemp{a,b} = TOI.(comparisons{a,2}){b};
            end
            ...
            for b = 1:size(TOI,1) % loop over TOIs
                saveNameTemp{a,b} = [comparisons{a,1}{1,1},'_',comparisons{a,1}{1,2},'_VS_',comparisons{a,2}{1,1},'_',comparisons{a,2}{1,2},'_TOI_',num2str(TOI{b,1}(1,1)),'_',num2str(TOI{b,1}(1,2)),'_',dataAv];
                condition1Temp{a,b} = comparisons{a,1};
                condition2Temp{a,b} = comparisons{a,2};
                toiRangeTemp{a,b} = TOI{b};
            end
