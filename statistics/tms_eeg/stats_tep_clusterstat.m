%##### COMPARE TEPS BETWEEN BASELINE PFC AND PAR #####

% This script compares baseline TEPs averaged across conditions between the
% PFC and the PAR stimulation sites using cluster-based permutation
% statistics as implemented in FieldTrip. Data are compared across a broad
% time window (15 to 250 ms). 
...
dataAv = 'N14';
dataZ = ''; % '' | '_zscore_time' | '_zscore_space'
...
% TOI = {[0.031,0.05]}; % All time
% % Comparisons of interest
% comparisons = {{'C1','pfc','T0'},{'C2','pfc','T0'};...
%     {'C1','ppc','T0'},{'C2','ppc','T0'}};

% +++++ BETWEEN SITE COMPARISONS ++++++
% % Time window of interest
% TOI = {[0.015,0.03]}; % All time
% TOI = {[0.031,0.05]}; % All time
% TOI = {[0.015,0.05]}; % All time
% TOI = {[0.015,0.25]}; % All time
% % Comparisons of interest
% comparisons = {{'C1','pfc','T0'},{'C1','ppc','T0'};...
%     {'C2','pfc','T0'},{'C2','ppc','T0'}};
% comparisons = {{'C1','pfc','T0'},{'C1','ppc','T0'}};

% +++++ BETWEEN SITE COMPARISONS - AVERAGE OVER BASELINES ++++++
%Time window of interest
% TOI = {[0.015,0.03]}; % All time
% TOI = {[0.031,0.045]}; % All time
% TOI = {[0.015,0.05]}; % All time
% TOI = {[0.015,0.06]}; % All time
% TOI = {[0.015,0.25]}; % All time
% Comparisons of interest
% comparisons = {{'average','pfc','T0'},{'average','ppc','T0'}};
...
% % Comparisons of interest - between condition
...
neighbour = load([fileRoot,'\Science\Scripts\projects\NMDA_TMS_EEG\neighbour_template_nmda.mat']);
